...
REGISTER_ACTOR_CLASS( SpriteAsync );
...
SpriteAsync::SpriteAsync( const SpriteAsync &cpy ):
        Sprite( cpy ) //It's not supposed to be this but I don't care I'm not using the copy constructor anyways
{
...
//No I don't know why it doesn't work someone fix it
...
void SpriteAsync::LoadFromTexture(RageTextureID ID) {
    TEXTUREMAN->AdjustTextureID(ID); //Adjusts color depth and max size and mipmaps
...
//wat -RL
//If some genius calls this while the previous thread is still loading, it will cause a memory leak because an image is created and not allocated to this sprite
//
void SpriteAsync::Load( RageTextureID ID)
{
    UnloadTexture(); //Always free previous texture
...
//If using Texture= in a lua table.
void SpriteAsync::LoadFromNode( const XNode* pNode )
{
    RString sPath;
...
void SpriteAsync::SetTexture( RageTexture *pTexture )
{
...
void SpriteAsync::UnloadTexture()
{
...
void SpriteAsync::LoadStatesFromTexture()
{
...
// Everything below this is just copypasted code from Sprite class
// because I couldn't figure out why it's not drawing

void SpriteAsync::UpdateAnimationState()
{
...
void SpriteAsync::Update( float fDelta )
{
...
void SpriteAsync::DrawTexture( const TweenState *state )
{
...
bool SpriteAsync::EarlyAbortDraw() const
{
    return m_pTexture == nullptr;
}

void SpriteAsync::DrawPrimitives()
{
...
void SpriteAsync::EnableAnimation( bool bEnable )
{
...
void SpriteAsync::SetAllStateDelays(float fDelay)
{
...
int SpriteAsync::GetNumStates() const
{
    return m_States.size();
}

void SpriteAsync::SetState( int iNewState )
{
    /*
...
void SpriteAsync::RecalcAnimationLengthSeconds()
{
...
void SpriteAsync::SetSecondsIntoAnimation( float fSeconds )
{
...
RString	SpriteAsync::GetTexturePath() const
{
    if( m_pTexture==NULL )
        return RString();
...
void SpriteAsync::SetCustomTextureRect( const RectF &new_texcoord_frect )
{
...
void SpriteAsync::SetCustomTextureCoords( float fTexCoords[8] ) // order: top left, bottom left, bottom right, top right
{
    m_bUsingCustomTexCoords = true;
    m_bTextureWrapping = true;
    for( int i=0; i<8; i++ )
...
void SpriteAsync::SetCustomImageRect( RectF rectImageCoords )
{
...
void SpriteAsync::SetCustomImageCoords( float fImageCoords[8] )	// order: top left, bottom left, bottom right, top right
{
...
void SpriteAsync::SetCustomPosCoords( float fPosCoords[8] )	// order: top left, bottom left, bottom right, top right
{
...
const RectF *SpriteAsync::GetCurrentTextureCoordRect() const
{
    return GetTextureCoordRectForState( m_iCurState );
}

const RectF *SpriteAsync::GetTextureCoordRectForState( int iState ) const
{
...
void SpriteAsync::GetActiveTextureCoords( float fTexCoordsOut[8] ) const
{
...
void SpriteAsync::StopUsingCustomCoords()
{
    m_bUsingCustomTexCoords = false;
}

void SpriteAsync::StopUsingCustomPosCoords()
{
    m_bUsingCustomPosCoords = false;
}

void SpriteAsync::SetTexCoordVelocity(float fVelX, float fVelY)
{
    m_fTexCoordVelocityX = fVelX;
    m_fTexCoordVelocityY = fVelY;
}

void SpriteAsync::ScaleToClipped( float fWidth, float fHeight )
{
    ...
    if( fWidth != -1 && fHeight != -1 )
    {
        ...
        SpriteAsync::StopUsingCustomCoords();

        // first find the correct zoom
        SpriteAsync::ScaleToCover( RectF(0, 0, fWidth, fHeight) );
...
void SpriteAsync::CropTo( float fWidth, float fHeight )
{
    m_fRememberedClipWidth = fWidth;
    m_fRememberedClipHeight = fHeight;
...
void SpriteAsync::StretchTexCoords( float fX, float fY )
{
    float fTexCoords[8];
...
void SpriteAsync::AddImageCoords( float fX, float fY )
{
    float fTexCoords[8];
    GetActiveTextureCoords( fTexCoords );
...
class LunaSpriteAsync: public Luna<SpriteAsync>
{
public:
...
LUA_REGISTER_DERIVED_CLASS( SpriteAsync, Sprite )
// lua end
